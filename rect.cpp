...
bool Rect::isOverlapping(const Rect &r) const {
    ...
    if(r.getLeftX() + r.getWidth() > getLeftX() && r.getBottomY() > getTopY() && r.getLeftX() + r.getWidth() < getRightX() && r.getTopY() < getBottomY()) {
        return true;
    }
...
void Rect::draw() const {
    ...
    glRectf(getLeftX(), getTopY(), getRightX(), getBottomY());
}